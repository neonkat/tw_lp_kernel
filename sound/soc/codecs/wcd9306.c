...
static int tapan_volatile(struct snd_soc_codec *ssc, unsigned int reg)
{
	...
	if (tapan_is_digital_gain_register(reg))
 	...
 	/* HPH gain registers */
 	if (reg == TAPAN_A_RX_HPH_L_GAIN || reg == TAPAN_A_RX_HPH_R_GAIN)
 		return 1;
#endif
...
static int tapan_codec_iir_mux_event(struct snd_soc_dapm_widget *w,struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = w->codec;
	pr_debug("%s: event = %d\n", __func__, event);
	switch (event)
...
static int tapan_codec_set_iir_gain(struct snd_soc_dapm_widget *w,struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = w->codec;
	int value = 0;
	pr_debug("%s\n", __func__);
